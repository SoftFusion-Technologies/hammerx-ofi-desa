 ...
 *
 * Descripción:
 * Modal para dar de baja un cliente adherido del módulo Débitos Automáticos.
 * Permite indicar fecha de baja y motivo/observación interna antes de ejecutar
 * el endpoint específico de baja.
 *
 * Tema: Baja de cliente - Débitos Automáticos
 * Capa: Frontend
 */

/* Benjamin Orellana - 31/03/2026 - Modal específico para baja operativa del cliente, separado del cambio de estado general para mantener un flujo claro y seguro */
const backdropV = {
  hidden: { opacity: 0 },
  visible: { opacity: 1 },
  exit: { opacity: 0 }
...
const ModalClienteBaja = ({ open, onClose, cliente, onSaved }) => {
  const [fechaBaja, setFechaBaja] = useState(todayInputValue());
  ...
  useEffect(() => {
    if (!open) return;
    setFechaBaja(
      row?.fecha_baja ? String(row.fecha_baja).slice(0, 10) : todayInputValue()
    );
    setMotivoBaja('');
    setSubmitting(false);
    setError('');
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fechaBaja) {
      setError('Debes ingresar la fecha de baja.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        observaciones_internas: motivoBaja.trim()
      };

      const response = await api.put(
        `/debitos-automaticos-clientes/${row.id}/dar-baja`,
        payload
      );
      ...
      setSuccessMsg('Cliente dado de baja correctamente.');
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[1460] flex items-center justify-center p-3 sm:p-5"
        variants={backdropV}
        initial="hidden"
        animate="visible"
        exit="exit"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        disabled={submitting}
                        className="h-12 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-700 outline-none transition-all duration-200 focus:border-red-400 focus:ring-4 focus:ring-red-100 disabled:cursor-not-allowed disabled:bg-slate-50"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <textarea
                        ...
                        disabled={submitting}
                        rows={6}
                        placeholder="Ej.: el cliente decidió no continuar, tarjeta dada de baja, cobro manual definitivo, etc."
                        className="w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 py-3 text-sm text-slate-700 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-red-400 focus:ring-4 focus:ring-red-100 disabled:cursor-not-allowed disabled:bg-slate-50"
                      />
                    </div>
                  </div>

                  <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                    Esta acción cambiará el estado operativo del cliente y
                    dejará registrada la fecha de baja.
                  </div>
...
export default ModalClienteBaja;
