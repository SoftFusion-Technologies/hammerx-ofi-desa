 ...
 *
 * Descripción:
 * Modal para aprobar un período mensual del módulo Débitos Automáticos.
 * Permite confirmar fecha de resultado y dejar observaciones operativas.
 *
 * Tema: Aprobar período - Débitos Automáticos
 * Capa: Frontend
 */

/* Benjamin Orellana - 01/04/2026 - Modal específico para aprobar el período y dejar fecha de resultado más observación operativa antes de ejecutar el endpoint /aprobar */
const backdropV = {
  hidden: { opacity: 0 },
  visible: { opacity: 1 },
  exit: { opacity: 0 }
...
const ModalPeriodoAprobar = ({ open, onClose, periodo, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      /* Benjamin Orellana - 01/04/2026 - Payload mínimo y razonable para aprobación; si el controlador espera otro naming puntual, solo se ajustan estas keys */
      const payload = {
      ...
      const response = await api.put(
        `/debitos-automaticos-periodos/${row.id}/aprobar`,
        payload
      );
      ...
      setSuccessMsg('Período aprobado correctamente.');
      ...
      setError(
        err?.response?.data?.mensajeError || 'No se pudo aprobar el período.'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[1500] flex items-center justify-center p-3 sm:p-5"
        variants={backdropV}
        initial="hidden"
        animate="visible"
        exit="exit"
          ...
          <form onSubmit={handleSubmit} className="px-4 py-4 sm:px-6 sm:py-5">
                ...
                <div className="grid grid-cols-1 gap-3">
                  ...
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-3">
                    ...
                    <p className="mt-1 text-sm font-semibold leading-6 text-emerald-700">
                      Recordá cobrar el contrato en SocioPlus si esta aprobación
                      corresponde al cierre operativo del mes.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        disabled={submitting}
                        className="h-12 w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-700 outline-none transition-all duration-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:bg-slate-50"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <textarea
                        ...
                        disabled={submitting}
                        rows={6}
                        placeholder="Observación interna del cobro aprobado..."
                        className="w-full rounded-2xl border border-slate-200 bg-white pl-10 pr-4 py-3 text-sm text-slate-700 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:bg-slate-50"
                      />
                    </div>
                  </div>

                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                    Esta acción dejará el período como cobrado y,
                    operativamente, contará como mes aprobado para el cliente.
                  </div>
            ...
            <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-5 text-sm font-semibold text-white transition-all duration-200 hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-70"
              >
                {submitting ? (
                  <>
...
export default ModalPeriodoAprobar;
