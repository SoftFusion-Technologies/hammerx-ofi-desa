/* 
Hecho por: Sergio Manrique
Fecha: 21/01/2026
Descripción: Componente modal para mostrar la agenda de hoy del equipo de remarketing.
*/

import { useEffect, useState, useCallback } from 'react';
import { AnimatePresence, motion } from 'framer-motion';

export default function AgendaDeHoyModalRemarketing({ open, onClose, userId, level }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(
        `http://localhost:8080/ventas-remarketing/agenda/hoy?${qs.toString()}`
      );
  ...
  const marcarDone = async (id) => {
    ...
    try {
      await fetch(`http://localhost:8080/ventas-remarketing/agenda/${id}/done`, {
        method: 'PATCH'
      });
    } catch {
      setItems(prev);
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex-shrink-0">
                            <button
                              ...
                              className="rounded-xl bg-[#fc4b08] px-4 py-2 text-sm md:text-[15px] font-bold text-white shadow hover:shadow-md hover:bg-[#e0480b] active:scale-95 transition"
                            >
                              Marcar realizado
                            </button>
                          </div>
