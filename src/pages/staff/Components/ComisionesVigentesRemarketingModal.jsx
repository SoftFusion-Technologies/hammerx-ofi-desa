...
export default function ComisionesVigentesRemarketingModal({
  open,
  onClose,
  userLevel = 'admin'
}) {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:8080/comisiones-vigentes-remarketing${qs}`);
  ...
  const onDelete = async (id) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8080/comisiones-vigentes-remarketing/${id}`,
        { method: 'DELETE' }
      );
  ...
  const onSaveEdit = async () => {
    ...
    try {
      ...
      const url = isNew
        ? `http://localhost:8080/comisiones-vigentes-remarketing`
        : `http://localhost:8080/comisiones-vigentes-remarketing/${editing.id}`;
